...
public class Solution1949_D4_등산로 {
			...
			for(int i=0;i<n;i++) {
				for(int j=0;j<n;j++) {
					for(int dig=1;dig<=k;dig++) {
						map[i][j] = map[i][j] - dig;
						for(int s=0;s<spIdx;s++) {
							if(i*n+j == startPoint[s]) {
								continue;
							}
							visit[startPoint[s]/n][startPoint[s]%n] = 1;
							solve(startPoint[s]/n, startPoint[s]%n, 1);
							visit[startPoint[s]/n][startPoint[s]%n] = 0;
						}
						map[i][j] = map[i][j] + dig;
					}
				}
	...
	public static void solve(int x, int y, int len) {
		...
		for(int d=0;d<4;d++) {
			int nx = x + dx[d];
			int ny = y + dy[d];
			if(inrange(nx,ny) && map[nx][ny] < map[x][y] && visit[nx][ny] == 0) {
				visit[nx][ny] = 1;
				solve(nx,ny,len+1);
				visit[nx][ny] = 0;
			}
